...
```c
bool ao_num64_wait     (ao_num64_wait_t * w, ao_time_t timeout);
bool ao_num64_wait_from(ao_num64_wait_t * w, ao_time_t timeout, ao_time_t beginning);
```
...
```c
bool ao_num64_wait_forever(ao_num64_wait_t * w);
```
...
```c
bool ao_num64_wait_try(ao_num64_wait_t * w);
```
