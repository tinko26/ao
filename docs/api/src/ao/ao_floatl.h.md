...
This module defines the fastest floating-point type available on the target platform to be the `long double` type. Additionally, it provides a couple of mathematical functions for that type.

An override of the [`ao_float.h`](ao_float.h.md) module can conveniently include this module, if actually `long double` is the fastest floating-point type available on the target platform.
