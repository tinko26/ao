...
```c
#define AO_UART_BAUD_MAX_5 AO_UART_BAUD_MAX(AO_SYS_CLOCK_UART5)
#define AO_UART_BAUD_MIN_5 AO_UART_BAUD_MIN(AO_SYS_CLOCK_UART5)
```
...
```c
#define AO_UART_BAUD_HIGH_MAX_5 AO_UART_BAUD_HIGH_MAX(AO_SYS_CLOCK_UART5)
#define AO_UART_BAUD_HIGH_MIN_5 AO_UART_BAUD_HIGH_MIN(AO_SYS_CLOCK_UART5)
```
...
```c
#define AO_UART_BAUD_LOW_MAX_5 AO_UART_BAUD_LOW_MAX(AO_SYS_CLOCK_UART5)
#define AO_UART_BAUD_LOW_MIN_5 AO_UART_BAUD_LOW_MIN(AO_SYS_CLOCK_UART5)
```
...
Configurates the baud rate. This function sets up the baud rate, that has been configured by `AO_UART_BAUD_5`, and enables or disables high-speed mode, depending on the specific value. This function is attributed as weak. Hence, the application can provide another implementation.
...
Configurates the frame characteristics. This function sets up the numbers of data bits and stop bits as well as the parity, that have been configured in by `AO_UART_DATA_BITS_PARITY_5` and `AO_UART_STOP_BITS_5`. This function is attributed as weak. Hence, the application can provide another implementation.
...
Configurates the loopback mode. This function enables or disable loopback mode, depending on the configuration by `AO_UART_LOOPBACK_5`. This function is attributed as weak. Hence, the application can provide another implementation.
